...
def get_theta_k_pairwiseBinomial(k, data_, num_steps_autograd = 10000):

    subdata_ = get_subdata(data_, k)
    
    mdl = nll_l_cvm(subdata_)
    optimizer = torch.optim.AdamW(mdl.parameters(), lr=1e-3)
    
    for steps in range(num_steps_autograd):  
        loss = mdl() 
        optimizer.zero_grad(set_to_none=True)
        loss.backward()
        optimizer.step()
    
    normalized_theta_hat_mle_SUB = normalize(mdl.theta_layer.weight.transpose(1,0).detach().numpy())
    unnormalized_theta_hat_mle_SUB = mdl.theta_layer.weight.transpose(1,0).detach().numpy()
    #return(normalized_theta_hat_mle_SUB[:,1])
    return(normalized_theta_hat_mle_SUB[:,1])
