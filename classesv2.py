...
def CELL_minQ_kn(k,C, V, mu_vec, verbose = False, solver = 'MOSEK'):
...
def get_param_path_V_epoch(data, epoch_range,thetaTRUE_mat):
    '''as epochs increase what happens to theta matrix, element by element.(To show contraction)
    '''
    mdrModel = MDR_w_backfitting(textData_obj= data)
    grid_len = len(epoch_range)
    num_param = thetaTRUE_mat.shape[0] * thetaTRUE_mat.shape[1]
    thetaHat_for_diff_epochs = np.empty([num_param,0])
    thetaTRUE_vec = thetaTRUE_mat.reshape(num_param)

    for j in epoch_range:
        print( "num_epoch: " + str(j))

        theta_hat, mu_hat = mdrModel.fit(num_epochs=j, num_iter = num_iter)

        t = theta_hat.reshape(-1,1)
        thetaHat_for_diff_epochs = np.append(thetaHat_for_diff_epochs, t, axis = 1)

    true_path = np.repeat(thetaTRUE_vec,grid_len, axis = 0 ).reshape(num_param, grid_len)


    for p in range(num_param):
        item_path = thetaHat_for_diff_epochs[p,]
        plt.plot(epoch_range, true_path[p,])
        plt.plot(epoch_range, item_path)
        plt.title('Parameter : ' + str(p))
        plt.xlabel('number of backfitting repetitions')
        plt.ylabel('Value of parameter')
        plt.legend(['Parameter Value', 'theta_hat()'])
        plt.show()
    return()


def get_avgErrSq_path_V_epoch(data, epoch_range,thetaTRUE_mat):
    '''as epochs increase what happens to average(among thetas)error squared.(To show contraction)
    '''
    ...
    errSq = (true_path - thetaHat_for_diff_epochs)**2
    
    err_path = np.mean(errSq, axis=0)
    plt.plot(epoch_range, err_path)
    plt.title('Error path vs Epoch Range')
    plt.xlabel('number of backfitting repetitions')
    plt.ylabel('Average err sq / Frobenius norm of Err matrix')
    plt.show()
    return()
